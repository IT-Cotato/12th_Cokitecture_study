...
auto-increment 속성이 설정된 관계형 데이터베이스의 기본 키?

→ 분산 환경에서는 이 접근법 안 통함

- 데이터베이스 서버 한 대로는 요구를 감당할 수 없음
- 여러 데이터베이스 쓰는 경우 지연 시간을 낮추기가 매우 힘듬
...
- 티켓 서버(ticket server)
- 트위터 스노플레이트(twitter snowflake) 접근법
...
UUID 예시: 09c93e62-50b4-468d-bf8a-c07e1040bfb2

UUID는 서버 간 조율 없이 독립적으로 생성 가능하다.

![image.png](images/image7_1.png)
...
![image.png](images/image7_3.png)

- sign 비트: 1비트를 할당한다. 음수와 양수를 구별하는 데 사용할 수 있을 것이다.
- 타임스탬프: 41비트를 할당
    - 기원 시각(epoch)이후로 몇 밀리초(millisecond)가 경과했는지를 나타내는 값
    - 기원 시각으로 트위터 스노플레이크 구현에서 사용하는 값 1288834974657(Nov 04, 2010, 01:42:54 UTC에 해당)을 이용할 것이다.
- 데이터 센터 ID: 5비트를 할당
    - 2^5 = 32개 데이터센터를 지원할 수 있다.
- 서버 ID: 5비트를 할당
    - 데이터센터당 32개 서버를 사용할 수 있다.
- 일련번호: 12비트를 할당
...
### ID 구조를 따르는 값의 이진 표현 형태 → UTC 시각 추출

![image.png](images/image7_4.png)

이 방법을 역으로 적용하면 어떤 UTC 시각도 상술한 타임스탬프 값으로 변환할 수 있다.

41비트로 표현할 수 있는 타임스탬프의 최댓값은 2^41-1 = 2199023255551밀리초이다.

→ 대략 69년에 해당(2199023255551밀리초/1000/365일/24시간/3600초)

→ 이 ID 생성기는 69년동안만 정상 동작
